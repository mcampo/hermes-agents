...
The PDF is encrypted. Password = `${MERCADO_PAGO_PDF_PASSWORD}`.
...
doc = fitz.open(path)
if doc.is_encrypted:
    doc.authenticate(os.environ["MERCADO_PAGO_PDF_PASSWORD"])
text = _open_pdf_text(path, password=os.environ["MERCADO_PAGO_PDF_PASSWORD"])
```
